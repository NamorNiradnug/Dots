...
void Chunk::drawDots(QPainter *painter, Dots *dots)
{
    ...
    {
        painter->setPen(settings->colors()[player(dot(track.p1().x() % 16,
                                                      track.p1().y() % 16))]);
    ...
    for (int x = 0; x < 16; x++)
    {
        for (int y = 0; y < 16; y++)
        {
            painter->setBrush(settings->colors()[player(dot(x, y))]);
            ...
            if (dots->isFirst(QPoint(x, y)))
            {
                ...
                painter->setPen(settings->colors()[player(dot(x, y))]);
                painter->drawEllipse(dotCoord(x, y), 6, 6);
                painter->setPen(Qt::transparent);
            }
            fill_brush.setColor(settings->colors()[player(dot(x, y))]);
            ...
            fill_brush.setColor(settings->colors()[eater(dot(x, y))]);
            painter->setBrush(fill_brush);
            QPoint poly[4] = {
                dotCoord(x, y - 1),
                dotCoord(x + 1, y),
