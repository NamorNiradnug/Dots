...
nigh_points_func = [point_up, point_up_left, point_left, point_up_right, point_down_left, point_right, point_down_right,
                    point_down]
...
class Dots:
    ...
    def have_points_connect(self, point1, point2, used_tracks={1}, track_len=0):
        ...
        for i in npf:
            try:
                if self.point_map[(i(point1))[1]][(i(point1))[0]] == self.point_map[point1[1]][point1[0]] and not (
                        (tuple(i(point1)), point1) in used_tracks):
                    print(used_tracks, (point1, point2))
                    variable = self.have_points_connect(tuple(i(point1)), point2,
                                                        used_tracks and {(tuple(i(point1)), point2),
                                                                         (point2, tuple(i(point1)))}, track_len + 1)
                    result += variable[0]
    ...
    def draw_point(self, point_type, x, y):
        self.paper.create_line((x * 16 + 8) * self.scale, y * self.scale * 16, (x * 16 + 8) * self.scale,
                               (y + 1) * self.scale * 16)
        self.paper.create_line(x * self.scale * 16, (y * 16 + 8) * self.scale, (x + 1) * self.scale * 16,
                               (y * 16 + 8) * self.scale)
        if point_type != 0:
            self.paper.create_oval((x * 16 + 5) * self.scale, (y * 16 + 5) * self.scale, (x * 16 + 11) * self.scale,
                                   (y * 16 + 11) * self.scale, fill=self.players_colors[point_type])
            ...
            for j in range(len(self.point_map[i][x:x + w:])):
                try:
                    if self.point_map[y:y + h:][i][x:x + w:][j] == self.point_map[y:y + h:][i][x:x + w:][j + 1] > 0:
                        self.paper.create_line((j * 16 + 8) * self.scale, (i * 16 + 8) * self.scale,
                                               (j * 16 + 24) * self.scale, (i * 16 + 8) * self.scale,
                                               fill=self.players_colors[self.point_map[y:y + h:][i][x:x + w:][j]],
                                               width=2 * self.scale)
                except:
                    pass
                try:
